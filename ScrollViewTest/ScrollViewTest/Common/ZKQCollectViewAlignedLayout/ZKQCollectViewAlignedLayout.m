...
@interface ZKQCollectViewAlignedLayout ()

@end

@implementation ZKQCollectViewAlignedLayout
        ...
        if (!attributes.representedElementKind) {
            ...
            if (attributes.frame.origin.x == [self evaluatedSectionInsetForItemAtIndex:section].left ) {
                if(left != -1) {
                    ...
                    switch (_alignType) {
                        case ZKQCollectViewAlignLeft:
                            break;
                        case ZKQCollectViewAlignRight:
                            offset += self.collectionView.frame.size.width - width;
                            break;
                        case ZKQCollectViewAlignMiddle:
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.alignType == ZKQCollectViewAlignLeft) {
        if (indexPath.row == 0) {
...
#pragma mark 暂时用不到就没写
    } else if (self.alignType == ZKQCollectViewAlignMiddle) {
        
    } else if (self.alignType == ZKQCollectViewAlignRight) {
